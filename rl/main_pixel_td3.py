"""
Main file for TD3 from raw pixels.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy_name", default="Pixel-TD3")           # Policy name
    ...
    parser.add_argument("--replay_size", default=1e5, type=int)         # Size of replay buffer
                                                                        # (100K to fit in memory, use --replay_type disk for larger)

    parser.add_argument("--arch", default="ilya_bn")                    # which network architecture to use
    ...
    parser.add_argument("--replay_type", default="dataset")             # which replay implementation to use

    args = parser.parse_args()
    args.save_models = not args.no_save_models

    if args.name is None:
        args.name = "Pixel{}_{}".format(args.env_name, args.policy_name)
    ...
    # switch to pixel observations and log the args
    ...
    # Initialize policy
    policy = PixelTD3.PixelTD3(state_dim, action_dim, max_action,
            arch=args.arch, img_width=args.img_width, stack=args.stack)
    policy.mode('eval')

    if args.replay_type == 'dataset':
        # faster for images than ReplayBuffer, but still in memory
        warnings.warn((
            "WARNING: replay buffer will attempt to store {} images in memory.\n"
            "You can reduce the replay buffer size with --replay_size n. "
            "If you have an SSD, you can use a disk-backed replay instead with --replay_type disk."
        ).format(args.replay_size))
        replay_buffer = utils.ReplayDataset(max_size=args.replay_size)
    elif args.replay_type == 'disk':
        # SSD >> HDD for this, lots of random access
